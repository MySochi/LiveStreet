...
{if $oUserProfile->getId() == '1' } <br> Учётная запись администратора проекта {else}
	...
	<div class="wrapper">
		...
		<div class="profile-right">
			...
			{if $aUserFieldContactValues}
				...
				<ul class="profile-contact-list">
					{foreach from=$aUserFieldContactValues item=oField}
						<li>
							<span class="webfonts">{$oField->getName()}</span>
							<span class="link">{$oField->getValue(true,true)} </span>
						</li>
			...
			{if $aUserFieldContactValues}
				...
				<ul class="profile-contact-list">
					{foreach from=$aUserFieldContactValues item=oField}
						<!-- <li class="webfonts">{$oField->getName()}</li> -->
						<!-- <li><i class="icon-contact icon-contact-{$oField->getName()}" title="{$oField->getName()}"></i> {$oField->getValue(true,true)}</li> -->
						<li>
							<span class="webfonts">{$oField->getName()}</span>
							<span class="link">{$oField->getValue(true,true)} </span>
