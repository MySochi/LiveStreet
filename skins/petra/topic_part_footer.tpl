	...
	<footer class="topic-footer">
		...
		<ul class="topic-info">
			<li class="topic-info-author-avatar">
			...
			<li class="topic-info-share" data-topic-id="{$oTopic->getId()}" onclick="jQuery('#topic_share_{$oTopic->getId()}').slideToggle(); return false;">
				<a title="{$aLang.topic_share}">&#0064;</a>
			</li>
			
			<li class="topic-info-favourite" onclick="return ls.favourite.toggle({$oTopic->getId()},$('#fav_topic_{$oTopic->getId()}'),'topic');">
				<a id="fav_topic_{$oTopic->getId()}" class="favourite {if $oUserCurrent && $oTopic->getIsFavourite()}active{/if}">&#0116;</a>
